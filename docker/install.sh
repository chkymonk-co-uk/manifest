...
#
# Downloads the Docker Compose file, generates a BETTER_AUTH_SECRET, writes
# a .env, and brings up the stack. Designed for first-time self-hosters who
# want a one-command setup.
#
# Usage:
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  printf '    \033[2m$ cat > %s <<EOF ... EOF\033[0m\n' "$ENV_PATH"
else
  umask 077
  cat > "$ENV_PATH" <<EOF
...
for _ in $(seq 1 24); do
  ...
  Change the seeded password after your first login, then set SEED_DATA=false
  in $ENV_PATH and recreate: (cd $INSTALL_DIR && docker compose up -d)
EOF
    exit 0
  fi
  sleep 5
